...
class AbstractChart extends Component {
  ...
  renderVerticalLabels = config => {
    ...
    return labels.map((label, i) => {
      return (
        <Text
          onPressIn={() => onDataPointClick({label})}
          key={Math.random()}
          x={((width / labels.length) * i + 10 + horizontalOffset) * fac}
          y={(height * 3) / 4 + fontSize * 2}
          fontSize={fontSize}
  ...
  renderVerticalLines = config => {
    ...
    return [...new Array(data.length)].map((_, i) => {
      return (
        <Line
          key={Math.random()}
          x1={Math.floor((width / data.length) * i) + 10}
          y1={10}
          x2={Math.floor((width / data.length) * i) + 10}
          y2={height - height / 4 + paddingTop}
          stroke={labels[i] === clickedPoint ? 'black' : 'transparent'}
          strokeDasharray="5, 7"
          strokeWidth={1}
        />
      );
